...
Canny_track_names=["lower_threshold","upper_threshold"]
...
#_______________Canny Edge Detection____________#
    if canny:
        ...
        cannyIm = cv2.Canny(cv2.equalizeHist(disp_gray),lower_threshold,upper_threshold,apertureSize = aper)

        contours=cv2.findContours(cannyIm.copy(),cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
        contours=imutils.grab_contours(contours)
        cv2.drawContours(rect_bound, contours, -1, (255,0,0), 3)
        for contr in contours:
            M=cv2.moments(contr)
